...
# --- 1. 개발자 모드 로직 수정 ---
# st.session_state를 사용하여 개발자 모드 활성화 상태를 기억
...
with st.sidebar:
    ...
    if "passwords" in st.secrets and password == st.secrets.passwords.dev_mode_password:
        ...
        if st.button("결과 페이지 바로보기 (기본 버전)"):
            # 버튼을 누르면 세션 상태를 True로 변경하고 새로고침
            st.session_state.dev_mode_activated = True
            st.rerun()
    
    # 개발자 모드가 활성화된 상태일 때 비활성화 버튼 표시
    if st.session_state.dev_mode_activated:
...
def display_results(df, dev_mode=False):
    # 결과 표시 로직
    if not dev_mode:
...
if st.session_state.dev_mode_activated:
    ...
    if df_dev is not None:
        ...
        display_results(df_dev, dev_mode=True)
    else:
        st.error("개발자 모드를 위해 default_data.csv 파일이 필요합니다.")
# 일반 사용자이면서 설문이 완료되었으면 결과 표시
elif 'responses' in st.session_state and st.session_state.responses and ('current_section' not in st.session_state or st.session_state.current_section >= len(section_list)):
     file_to_load = 'lite_data.csv' if '라이트' in st.session_state.version else 'default_data.csv'
     df_results = load_data(file_to_load)
     if df_results is not None:
         display_results(df_results)
# 일반 사용자이면서 설문 진행 중이면 설문 표시
else:
    display_survey()
