...
try:
    ...
    marquee_content = "조언 파일을 찾을 수 없습니다. advice_data.csv 파일을 확인해주세요."
except Exception:
    marquee_content = "조언을 불러오는 중 오류가 발생했습니다."
...
def display_survey():
    ...
    with st.form(key=f"form_{version}_{section_index}"):
        ...
        for _, row in questions_df.iterrows():
            ...
            st.radio("선택", [1, 2, 3, 4, 5], key=f"q_{row['번호']}", format_func=lambda x: options_map[x], horizontal=True, label_visibility="collapsed")
